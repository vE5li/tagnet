...
pub enum SyncDirectoryCommand {
    ...
    ReadFile {
        file_id: FileId,
        respond_to: tokio::sync::oneshot::Sender<Option<(PathBuf, Vec<u8>, String)>>,
    },
}
...
impl SyncDirectoryManager {
    pub async fn new(
        ...
        let sync_directories = configuration
            ...
            .filter_map(|sync_directory| {
                ...
                let database_path = sync_directory_db_path(&path);
    ...
    fn add_file(
        ...
        sync_directory
            .database
            .add_file(file_id, path.as_ref().to_string_lossy())
    ...
    fn handle_command(&mut self, command: SyncDirectoryCommand) -> Result<(), SyncDirectoryError> {
        match command {
            SyncDirectoryCommand::CreateFile {
                ...
                sync_directory
                    .database
                    .add_file(file_id, file_name.to_string_lossy())
                    .map_err(|_| SyncDirectoryError::FailedAddingFile)?;
            ...
            SyncDirectoryCommand::ReadFile {
                ...
                let mut response: Option<(PathBuf, Vec<u8>, String)> = None;
                for sync_directory in &self.sync_directories {
                    let file = match sync_directory.database.get_file(file_id) {
                        Ok(file) => file,
                        Err(_) => continue,
                    };
                    let (relative_path, absolute_path) = match &sync_directory.sync_type {
                        SyncType::Universal => (
                            PathBuf::from(file_id.to_string()),
                            sync_directory.path.join(file_id.to_string()),
                        ),
                        SyncType::TagBased { .. } => (
                            PathBuf::from(&file.path),
                            sync_directory.path.join(&file.path),
                        ),
                    };
                    ...
                    let content_hash = Self::calculate_content_hash(&content);
                    response = Some((relative_path, content, content_hash));
                    break;
                }
